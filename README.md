...
### Backend Setup
```bash
# Assuming shared Ignet Flask API running on port 9637
# See Ignet repository for backend setup

# Test connectivity
curl http://localhost:9637/api/v1/vaccine/search?q=covid
```
...
Vignet is deployed on Vercel with the Flask backend on a dedicated Python server.
...
```
VITE_API_URL=https://ignet.org/api/v1
VITE_MCP_ENDPOINT=https://ignet.org/api/v1/mcp
```
...
- **Contact**: https://ignet.org/vignet/contact/
- **Report Issues**: https://ignet.org/vignet/report-issue/
